...
def _score_request(  # pragma: no cover
    ...
    *,
    ...
    return {"kind": "scores", "peak_rss_bytes": _peak_rss_bytes(), "scores": scores}
...
class IsolatedE5SemanticProvider:
    ...
    def score(
        ...
        if not isinstance(raw_scores, list) or len(raw_scores) > limits.max_response_entries:
            self._terminate()
            raise SemanticProviderInvalid("semantic provider response invalid")
        try:
            scores = tuple(SemanticScore.model_validate(value) for value in raw_scores)
    ...
    def _validate_request(
        ...
        if len(query.encode("utf-8")) + sum(sizes) > limits.max_total_text_bytes:
