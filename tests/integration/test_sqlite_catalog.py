...
def test_real_revision_one_catalog_upgrades_transactionally_to_current(tmp_path: Path) -> None:
    ...
    assert versions == tuple(range(1, CURRENT_SCHEMA_VERSION + 1))


def test_real_revision_four_catalog_upgrades_to_rich_revision_five(
    tmp_path: Path,
) -> None:
    """Upgrade the immediately prior released shape without changing its checksums."""
    path = tmp_path / "catalog.sqlite3"
    prior = MIGRATIONS[:4]
...
def test_reference_snapshot_includes_versions_and_terminal_jobs(tmp_path: Path) -> None:
    ...
    assert snapshot.catalog_schema_version == CURRENT_SCHEMA_VERSION
